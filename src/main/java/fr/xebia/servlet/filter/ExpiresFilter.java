...
public class ExpiresFilter implements Filter {
    ...
    public class XPrintWriter extends PrintWriter {
        ...
        public PrintWriter append(char c) {
            ...
            return ourOut.append(c);
        }

        @Override
        public PrintWriter append(CharSequence csq) {
            fireBeforeWriteResponseBodyEvent();
            return ourOut.append(csq);
        }

        @Override
        public PrintWriter append(CharSequence csq, int start, int end) {
            fireBeforeWriteResponseBodyEvent();
            return ourOut.append(csq, start, end);
        }
        ...
        public PrintWriter printf(Locale l, String format, Object... args) {
            fireBeforeWriteResponseBodyEvent();
            return ourOut.printf(l, format, args);
        }

        @Override
        public PrintWriter printf(String format, Object... args) {
            fireBeforeWriteResponseBodyEvent();
            return ourOut.printf(format, args);
        }
    ...
    private static Log logger=LogFactory.getLog(ExpiresFilter.class);
    ...
    protected Date getExpirationDate(HttpServletRequest request, XHttpServletResponse response) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(String.format("Use %s matching '%s' for content-type '%s'", configuration, matchingContentType, contentType));
        }
...
Enumeration<String> names = filterConfig.getInitParameterNames(); names.hasMoreElements();) {
    ...
    protected boolean isEligibleToExpirationHeaderGeneration(HttpServletRequest request, XHttpServletResponse response) {
        ...
        if (expirationHeaderHasBeenSet) {
            if (logger.isDebugEnabled()) {
                logger.debug(String.format("Request '%s' with response status '%s' content-type '%s', expiration header already defined", 
            ...
            if (response.getStatus() == skippedStatusCode) {
                if (logger.isDebugEnabled()) {
                    logger.debug(String.format(
                            "Request '%s' with response status '%s' content-type '%s', skip expiration header generation for given status",
                            request.getRequestURI(), response.getStatus(), response.getContentType()));
                }
                return false;
            }
    ...
    public void onBeforeWriteResponseBody(HttpServletRequest request, XHttpServletResponse response) {
        ...
        if (expirationDate == null) {
            if (logger.isDebugEnabled()) {
                logger.debug(String.format("Request '%s' with response status '%s' content-type '%s', no expiration configured",
                        request.getRequestURI(), response.getStatus(), response.getContentType()));
            }
        } else {
            if (logger.isDebugEnabled()) {
                logger.debug(String.format("Request '%s' with response status '%s' content-type '%s', set expiration date {}", 
